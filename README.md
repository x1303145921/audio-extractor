...
> [!IMPORTANT]
> ## ✅ v1.5.2 —— 当前最新版（仓库优化 · 修复 PWA 缓存未生效 · 新增一键下载）
>
...
>
> 👉 **新用户请直接下载 [最新版 v1.5.2](https://github.com/x1303145921/audio-extractor/releases/latest)，旧版本不推荐使用。**
...
| 版本 | 状态 | 说明 |
|---|---|---|
| **v1.5.2** | ✅ **当前最新版** | 修复 PWA 离线缓存未注册（Service Worker 生效）+ 新增「下载最新版.bat」一键下载脚本（国内多镜像自动回退）+ 清理过时打包脚本；含 v1.5.1 全部修复 |
| v1.5.1 | ✅ 可用版 | 补回便携包缺失的 Node 运行时 + 修复点目录路径下提取截断/下载 404 |
| v1.5.0 / v1.4.0–v1.4.2 | ⛔ 已下架 | 打包缺陷导致 zip 缺 node.exe（Release 已移除，git tag 保留）；v1.4.x 特性含错误分类提示/SSE 断线重连/统一错误格式，请使用 v1.5.2 |
...
下载 **最新版** `audio-extractor-portable-v1.5.2.zip`（见 [GitHub Releases](https://github.com/x1303145921/audio-extractor/releases)），解压后双击「**安装到桌面.bat**」：桌面自动出现带图标的「**音频提取工具**」，**以后双击桌面图标即可使用**（也可直接双击目录内的「启动工具-最小化.bat」）。
...
1. **下载**：到 [Releases](https://github.com/x1303145921/audio-extractor/releases) 页下载最新版 `audio-extractor-portable-v1.5.2.zip`（国内下载慢？见下方「🚀 国内下载加速」）。
...
A：v1.3.2 起已修复权限问题——若解压目录创建者与运行者权限不同（例如目录由管理员创建），旧版会因临时目录不可写而在转码过程中失败；新版会自动检测并回退到系统临时目录。**请升级到最新版（v1.5.2）**；若仍异常，确认解压目录未被杀软拦截。
