...
下载 **最新版** `audio-extractor-portable-v1.3.2.zip`（见 [GitHub Releases](https://github.com/x1303145921/audio-extractor/releases)），解压后双击「**启动工具-最小化.bat**」即可：服务在任务栏最小化窗口后台运行，浏览器自动打开（推荐）；或双击「启动音频提取工具.bat」使用带控制台窗口的经典方式。
...
3. **启动**：进入解压目录，双击「**启动工具-最小化.bat**」（推荐：任务栏一个最小化窗口，浏览器自动打开）或「**启动音频提取工具.bat**」（经典带控制台窗口方式）。
4. **首次运行提示**：若弹出 SmartScreen「已保护你的电脑」，点「**更多信息 → 仍要运行**」即可（便携包没有数字签名，属正常现象）。
5. **使用**：浏览器打开后，把视频文件拖进页面（或点击选择），选好输出格式点「提取音频」，完成后点「下载」。
6. **退出**：用完在页面底部点「**退出服务**」，后台进程即关闭；下次双击启动脚本即可再次使用。

> **想放在桌面？** 右键「启动工具-最小化.bat」→「发送到 → 桌面快捷方式」，以后双击桌面图标即可（也可以右键快捷方式 → 属性 → 更改图标，选用解压目录里 `public\app-icon.ico`）。
