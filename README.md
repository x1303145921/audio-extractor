...
| v1.5.0 / v1.4.0–v1.4.2 | ⛔ 已下架 | 打包缺陷导致 zip 缺 node.exe（Release 已移除，git tag 保留）；v1.4.x 特性含错误分类提示/SSE 断线重连/统一错误格式，请使用 v1.5.1 |
...
A：v1.3.2 起已修复权限问题——若解压目录创建者与运行者权限不同（例如目录由管理员创建），旧版会因临时目录不可写而在转码过程中失败；新版会自动检测并回退到系统临时目录。**请升级到最新版（v1.5.1）**；若仍异常，确认解压目录未被杀软拦截。
